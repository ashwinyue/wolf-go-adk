...
// 如果配置了 CozeLoop，则添加 CozeLoop 回调
// 设置环境变量：
// COZELOOP_WORKSPACE_ID=你的工作区ID
// COZELOOP_API_TOKEN=你的令牌
func AppendCozeLoopCallbackIfConfigured(_ context.Context) (closeFn CloseFn, startSpanFn StartSpanFn) {
	wsID := os.Getenv("COZELOOP_WORKSPACE_ID") // 使用 cozeloop 跟踪，来自 https://loop.coze.cn/open/docs/cozeloop/go-sdk#4a8c980e
	...
	if err != nil {
		log.Fatalf("cozeloop.NewClient 失败，错误: %v", err)
	}
